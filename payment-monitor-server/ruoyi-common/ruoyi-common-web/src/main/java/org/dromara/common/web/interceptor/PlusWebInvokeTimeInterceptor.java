...
public class PlusWebInvokeTimeInterceptor implements HandlerInterceptor {
    ...
    /**
     * 设备协议请求可能包含通知原文、配对信息或鉴权相关数据。
     * 这些路径只保留 URL 和耗时日志，不记录请求参数。
     */
    private static final String[] PARAMETER_LOG_DISABLED_PATH_PREFIXES = {
        "/api/v1/devices/",
        "/api/v1/device/",
    ...
    public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler) throws Exception {
        ...
        // 打印请求参数
        if (isParameterLoggingDisabled(request.getRequestURI())) {
            log.info("[PLUS]开始请求 => URL[{}],参数日志已关闭", url);
    ...
    static boolean isParameterLoggingDisabled(String requestUri) {
        if (StringUtils.isBlank(requestUri)) {
            return false;
        }
        for (String prefix : PARAMETER_LOG_DISABLED_PATH_PREFIXES) {
