...
public class SysProfileController extends BaseController {
    ...
    public R<Void> updateProfile(@Validated @RequestBody SysUserProfileBo profile) {
        ...
        if (StringUtils.isNotEmpty(profile.getEmail())
            ...
            return R.fail("登录邮箱必须通过邮箱验证流程修改");
        }
    ...
    public record SysUserPasswordBo(
        ...
        @jakarta.validation.constraints.Size(max = 64, message = "旧密码长度不能超过 64 位")
        String oldPassword,
        @NotBlank(message = "新密码不能为空")
        @jakarta.validation.constraints.Size(min = 12, max = 64, message = "新密码长度必须为 12–64 位")
        String newPassword) {
    }
