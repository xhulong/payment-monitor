...
public class SysUserController extends BaseController {
    ...
    public R<UserInfoVo> getInfo() {
        ...
        refreshLoginPermissions(loginUser);
    ...
    /**
     * 重新读取数据库中的角色、菜单和数据权限，并写回当前 Token 会话。
     * 角色在商户审核、成员邀请或岗位调整后发生变化时，刷新页面即可生效，
     * 无需通过重新登录重建 LoginUser。
     */
    private void refreshLoginPermissions(LoginUser loginUser) {
        Long userId = loginUser.getUserId();
        List<RoleDTO> roles = BeanUtil.copyToList(
            roleService.selectRolesByUserId(userId),
            RoleDTO.class
        );
        ...
        loginUser.setPosts(BeanUtil.copyToList(
            postService.selectPostsByUserId(userId),
            PostDTO.class
        ));
        if (loginUser.getRoleId() != null
            && roles.stream().noneMatch(role -> loginUser.getRoleId().equals(role.getRoleId()))) {
    ...
    public R<Void> resetPwd(@RequestBody SysUserBo user) {
        ...
        userService.checkUserDataScope(user.getUserId());
        user.setPassword(BCrypt.hashpw(user.getPassword()));
        int rows = userService.resetUserPwd(
            user.getUserId(),
            user.getPassword()
        );
        if (rows > 0) {
            ...
            stepUpTokenStore.revokeAll(user.getUserId());
